--EXEC [UspObterMatriculaDemonstrativoPagamentoPorId]

If Exists(Select * From sysobjects Where ID = OBJECT_ID(N'[dbo].[UspObterMatriculaDemonstrativoPagamentoPorId]') And OBJECTPROPERTY(ID, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[UspObterMatriculaDemonstrativoPagamentoPorId]
GO
...
CREATE PROCEDURE [dbo].[UspObterMatriculaDemonstrativoPagamentoPorId]
...
SET NOCOUNT ON
 ...
 --             ON MDPT.[IDTOTALIZADOR] = T.[ID]
          SELECT MDP.[GUID], MDP.[GUIDMATRICULA], MDP.[COMPETENCIA], MDP.[DATA_INCLUSAO], MDP.[DATA_ULTIMA_ALTERACAO],
                 MDP.[DATA_CONFIRMACAO],MDP.[IP_CONFIRMACAO],
                 M.[GUID], M.[MATRICULA], M.[DATA_ADMISSAO], M.[DATA_DEMISSAO], M.[DATA_INCLUSAO], M.[DATA_ULTIMA_ALTERACAO],
		         M.[DESCRICAO_CARGO], M.[DESCRICAO_SETOR], M.[FAIXA_IR], M.[FAIXA_SF], M.[GUIDCOLABORADOR], M.[GUIDEMPREGADOR],
		         M.[FORMA_PAGAMENTO], M.[BANCO], M.[AGENCIA], M.[CONTA], M.[DV_CONTA], M.[CARGA_HORARIA],
                 CONVERT(DECIMAL(11, 2), CONVERT(VARCHAR(MAX), M.[SALARIO_NOMINAL])) AS 'SALARIO_NOMINAL',
                 PF.[GUID], PF.[CPF], PF.[RG], PF.[DATA_NASCIMENTO], PF.[NOME], PF.[NUMERO_CTPS], PF.[SERIE_CTPS], PF.[UF_CTPS], PF.[GUIDPESSOA],
                 PJ.[GUID], PJ.[CNPJ], PJ.[DATA_FUNDACAO], PJ.[RAZAO_SOCIAL], PJ.[IDUNIDADE_NEGOCIO], PJ.[GUIDPESSOA],
		         MDPE.[GUID], MDPE.[GUIDMATRICULA_DEMONSTRATIVO_PAGAMENTO], MDPE.[IDEVENTO], MDPE.[REFERENCIA],
                 CONVERT(DECIMAL(8, 2), CONVERT(VARCHAR(MAX), MDPE.[VALOR])) AS 'VALOR_EVENTO',
				 E.[ID], E.[DATA_INCLUSAO], E.[DATA_ULTIMA_ALTERACAO], E.[DESCRICAO], E.[TIPO],
		         MDPT.[GUID], MDPT.[GUIDMATRICULA_DEMONSTRATIVO_PAGAMENTO], MDPT.[IDTOTALIZADOR],
				 CONVERT(DECIMAL(8, 2), CONVERT(VARCHAR(MAX), MDPT.[VALOR])) AS 'VALOR_TOTALIZADOR',
		         T.[ID], T.[DATA_INCLUSAO], T.[DATA_ULTIMA_ALTERACAO], T.[DESCRICAO]
            FROM [dbo].[MATRICULAS_DEMONSTRATIVOS_PAGAMENTO] AS MDP WITH(NOLOCK)
      INNER JOIN [dbo].[MATRICULAS] as M WITH(NOLOCK)
              ON MDP.[GUIDMATRICULA] = M.[GUID] 
